<?php
	...
	//link buat paging
	$linkaksi = 'med.php?mod=pengeluaran';
	...
	$aksi = 'mod/pengeluaran/act_pengeluaran.php';

	switch ($act) {
		default :
			echo"<div class='w3-container w3-small w3-pale-green w3-leftbar w3-border-green'>
				<h4 style='margin-bottom:0;padding-bottom:0;'>Data Pengeluaran</h4>
				<p style='margin-top:0;padding-top:0;'><i>Semua Data Pengeluaran Barang</i></p>
			</div>";
				...
				<tr>
					<td>
						<form class='w3-tiny' action='' method='GET'>	
							<input type='hidden' name='mod' value='pengeluaran'>
							<div class='w3-row'>
								<div class='w3-col s1'>
					...
					<td align='right'><a href='med.php?mod=pengeluaran' class='w3-btn w3-dark-grey w3-small'><i class='fa fa-refresh'></i> Refresh</a>
					<a href='med.php?mod=pengeluaran&act=form' class='w3-btn w3-small w3-blue'><i class='fa fa-file'></i> Tambah</a></td>
				</tr>
			...
			<table class='w3-table w3-striped w3-bordered w3-tiny w3-hoverable tbl'>
				...
				if($fd_kul > 0)
				{
					...
					while ($m = mysql_fetch_assoc($sql_kul)) {
						echo"<tr>
							...
							<td>$no</td>
							<td><a class='w3-text-blue w3-hover-text-red' href='med.php?mod=pengeluaran&act=detail&id=$m[no_faktur]'>$m[no_faktur]</a></td>
							<td>$m[nama_toko]</td>
							<td>$m[tgl_beli]</td>
							<td>$m[nama_kasir]</td>
							<td>".nama_petugas($m['petugas'])."</td>
							<td>".total_pengeluaran($m['no_faktur'])."</td>
							<td><a href='$aksi?mod=pengeluaran&act=hapus&id=$m[no_faktur]' onclick=\"return confirm('Yakin hapus data');\"><i class='fa fa-trash w3-large w3-text-red'></i></a>
							</td>
				...
				{
					echo"<tr>
						<td colspan='7'><div class='w3-center'><i>Data Pengeluaran Not Found.</i></div></td>
					</tr>";
				}
		...
		case "form" :
			echo"<div class='w3-container w3-small w3-pale-green w3-leftbar w3-border-green'>
				<h4 style='margin-bottom:0;padding-bottom:0;'>Form Transaksi Pengeluaran</h4>
				<p style='margin-top:0;padding-top:0;'><i>Input Data Pengeluaran Barang</i></p>
			</div>";
			...
			echo"<div class='w3-row-padding'>
				<div class='w3-col s3 w3-card'>
					...
					<div class='w3-card-2 w3-light-blue'>
						<form action='$aksi?mod=pengeluaran&act=add' method='POST' class='w3-container'>
							<input type='hidden' name='harga2' id='harga2'>
							<br><input type='text' name='nama_barang' id='nama_barang' placeholder='ketik nama barang ...' class='w3-input w3-tiny w3-border-0' required>
				...
				<div class='w3-col s9'>
					...
					$sql = mysql_query("SELECT a.*
										FROM tb_detail_pengeluaran_tmp a 
					...
					if(mysql_num_rows($sql) > 0)
					{
						...
						while($p = mysql_fetch_assoc($sql))
						{
							...
							echo"<tr>
								<td>$no</td>
								<td>$p[nama_barang]</td>
								<td>Rp. ".number_format($p['harga_beli'],0)." X $p[qty]</td>
								<td>Rp. ".number_format($sub_total)."</td>
								<td><a href='$aksi?mod=pengeluaran&act=batal&id=$p[nama_barang]' onclick=\"return confirm('Yakin ingin membatalkan?');\"><i class='fa fa-close w3-tiny w3-text-grey'></i></a></td>
							</tr>";
					...
					{
						echo"<tr>
							<td colspan='5'><i>Tidak ada pengeluaran</a><td>
						</tr>";
					}
					echo"</tbody>
						<tfoot>
					...
					<div class='w3-row-padding'>
						...
						<div class='w3-col s5'>
							<form action='$aksi?mod=pengeluaran&act=simpan' method='POST'>";
							$no_transaksi = no_pengeluaran_auto();
							echo "
								<input type='text' name='no_faktur' id='no_faktur' class='w3-input w3-tiny' value='$no_transaksi' readonly>

								<input type='text' name='nama_toko' id='nama_toko' class='w3-input w3-tiny' placeholder='ketik Nama Toko ...' required>
								...
								<p><button class='w3-btn w3-right w3-tiny' onclick=\"return confirm('Klik OK untuk melanjutkan');\"><i class='fa fa-save'></i> Simpan Pengeluaran</button></p>
							</form>
						</div>
		...
		case "detail" :
			...
			{
				echo"<div class='w3-container w3-small w3-pale-green w3-leftbar w3-border-green'>
					<h4 style='margin-bottom:0;padding-bottom:0;'>Detail Pengeluaran Barang</h4>
					<p style='margin-top:0;padding-top:0;'><i>Data Detail Pengeluaran Barang Baru</i></p>
				</div><br>
				...
				$sqltrans = mysql_query("SELECT * FROM tb_pengeluaran WHERE no_faktur = '$_GET[id]'") or die(mysql_error());
				...
				<p>
					...
					<a href='med.php?mod=pengeluaran&act=form' class='w3-btn w3-red w3-tiny'><i class='fa fa-cart-plus'></i> Transaksi Baru</a>
				</p>";
