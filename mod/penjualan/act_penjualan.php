...
<?php
	...
	if($mod == "penjualan" AND $act == "add")
	{
		...
		if (mysql_num_rows($cek_barang) > 0) {
			...
			if(mysql_num_rows($cek_det_barang) > 0)
			{
				...
				//mysql_query("UPDATE tb_barang SET jml_stok = jml_stok - 1 WHERE kode_barang = '$_GET[id]'") or die(mysql_error());
			}
			else
			{
	...
	elseif ($mod == "penjualan" AND $act == "batal") {
		...
		//update stok data barang
		$jmlqty = $_GET['jmlqty'];
		//mysql_query("UPDATE tb_barang SET jml_stok = jml_stok + $jmlqty WHERE kode_barang = '$_GET[id]'") or die(mysql_error());
		...
		if (mysql_num_rows($qtmp) > 0) {
			...
			if (($total_bayar <= $jmlbayar) OR ($_POST['status'] == "HUTANG")) {
				...
				else
				{
					...
					location.replace('../../med.php?mod=penjualan&act=printout&id=".$no_transaksi."');
					</script>";
				}
				//commit();
			}
	...
	elseif ($mod == "penjualan" AND $act == "hapus") {
		...
		else
		{
			...
			mysql_query("DELETE FROM tb_detail_penjualan WHERE no_transaksi = '$_GET[id]'") or die(mysql_error());
